...
from task_and_network.connectivity import Connectivity
...
def init_weights(C,num_units,distribution,rng):
    if C is not None:
        mask = C.plastic
        size = C.nplastic
    # distribution
    ...
    if C is not None:
        ...
        W[C.idx_plastic] = w
    return W.reshape((num_units, num_units))
...
def ei_Wrec(num_units,rng):
    rho0 = 1.5
    num_e = round(num_units*0.8)
    num_i = num_units - num_e

    ei_vector = np.ones(num_units)
    ei_vector[-num_i:] = -1

    # Default mask for recurrent weights
    exc, = np.where(ei_vector > 0)
    inh, = np.where(ei_vector < 0)
    C = np.zeros((num_units, num_units))
    for i in exc:
        C[i, exc] = 1
        C[i, i] = 0
        C[i, inh] = 1
    ...
    # Distribution
    Wrec_0 = init_weights(C,num_units, 'gamma',rng)
    settings = OrderedDict()
    Nexc = len(np.where(ei_vector > 0)[0])
    Ninh = len(np.where(ei_vector < 0)[0])
    ...
    if ei_vector is not None:
        Wrec_0_full = Wrec_0_full * ei_vector
    rho = spectral_radius(Wrec_0_full)
    ...
    Wrec_ = C.mask_plastic*Wrec_0 + C.mask_fixed
    Wrec_[Wrec_ < 0] = 0
    Wrec_ = Wrec_*ei_vector
    return Wrec_
...
class LeakyRNNCell(RNNCell):
    ...
    def __init__(self,
        ...
        w_rec0 = ei_Wrec(num_units, rng).T
...
#            print(v)
        ...
        for grad, var in self.grads_and_vars:
            self.grad_list.append(tf.reduce_max(grad))
        #---------------
        # gradient clipping
